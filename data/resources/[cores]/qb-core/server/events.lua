...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	if name == nil then 
		QBCore.Functions.Kick(src, 'Korjaa viisumisi (Steam)', setKickReason, deferrals)
        CancelEvent()
        return false
	end
	if(string.match(name, "[*%%'=`\"]")) then
        QBCore.Functions.Kick(src, 'Sinun viisumissa on ongelmana: ('..string.match(name, "[*%%'=`\"]")..') nämä eivät ole sallittuja (steam)', setKickReason, deferrals)
        CancelEvent()
        return false
	end
	if (string.match(name, "drop") or string.match(name, "table") or string.match(name, "database")) then
        QBCore.Functions.Kick(src, 'Korjaa viisumisi kuntoon (Steam)', setKickReason, deferrals)
        CancelEvent()
        return false
	end
    ...
    if (QBConfig.IdentifierType == "steam" and (steamid:sub(1,6) == "steam:") == false) then
        QBCore.Functions.Kick(src, 'Tarvitset viisumin (Steam)', setKickReason, deferrals)
        CancelEvent()
		return false
	elseif (QBConfig.IdentifierType == "license" and (steamid:sub(1,6) == "license:") == false) then
		QBCore.Functions.Kick(src, 'Tarvitset viisumin (Rockstar)', setKickReason, deferrals)
        CancelEvent()
		return false
    end
    ...
    if(not QBCore.Functions.IsWhitelisted(src)) then
        QBCore.Functions.Kick(src, 'Et ole veronmaksaja (Whitelist)', setKickReason, deferrals)
        CancelEvent()
        return false
    end
	deferrals.update("\nTarkistetaan jonoa..")
    if(QBCore.Config.Server.closed and not IsPlayerAceAllowed(src, "admin.join")) then
		QBCore.Functions.Kick(_source, 'Kaupungissa on myrskyvaroitus\n'..QBCore.Config.Server.closedReason, setKickReason, deferrals)
        CancelEvent()
        return false
	end
...
AddEventHandler('QBCore:UpdatePlayer', function(data)
	...
	if Player ~= nil then
		...
		TriggerClientEvent('QBCore:Notify', src, "Sait tilisiirron "..Player.PlayerData.job.payment)
...
AddEventHandler('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, "Olet nyt vapaalla.")
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, "Olet nyt työvuorossa.")
	end
	...
	QBCore.Functions.ExecuteSql(true, "SELECT * FROM `permissions`", function(result)
		if result[1] ~= nil then
			for k, v in pairs(result) do
				QBCore.Config.Server.PermissionList[v.steam] = {steam = v.steam, license = v.license, permission = v.permission, optin = true}
			end
		end
	end)
end)
